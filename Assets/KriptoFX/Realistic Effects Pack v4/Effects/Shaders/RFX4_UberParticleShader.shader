...
#ifdef USE_CUTOUT
	float cutout = UNITY_ACCESS_INSTANCED_PROP(_Cutout_arr, _Cutout);
	cutout = lerp(cutout, (1.001 - i.color.a + cutout), _UseParticlesAlphaCutout);

#ifdef USE_CUTOUT_TEX
	float mask = tex2D(_CutoutTex, i.noiseCutoutTexcoord.zw);
#else
	float mask = tex.a;
#endif

	float diffMask = mask - cutout;
	float alphaMask = lerp(saturate(diffMask * 10000) * res.a, saturate(diffMask * 2) * res.a, _UseSoftCutout);

#ifdef USE_CUTOUT_THRESHOLD
	float alphaMaskThreshold = saturate((diffMask - _CutoutThreshold) * 10000) * res.a;
