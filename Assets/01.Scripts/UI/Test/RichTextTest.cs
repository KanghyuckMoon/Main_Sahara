...
public class RichTextTest : MonoBehaviour
{
    ...
    [SerializeField] private bool enableTyping;
                ...
                if (this.text.supportRichText)
                {
                    ...
                    for (int i = 0; i < richTextSymbols.Length; i++)
                    {
                        ...
                        if (splitTypingTexts[typingIndex] == '<' &&
                            typingIndex + (1 + richTextSymbols[i].Length) < text.Length &&
                            text.Substring(typingIndex, 2 + richTextSymbols[i].Length).Equals(symbol))
                        {
                            switch (richTextSymbols[i])
                            {
                                case "b":
                                    ...
                                    {
                                        if (text.Substring(typingIndex, text.Length - typingIndex)
                                            .Contains(closeSymbol))
                                        {
                                            bold = true;
                                            symbolCatched = true;
                                        }
                    ...
                    if (splitTypingTexts[typingIndex] == '<' && typingIndex + 14 < text.Length &&
                        text.Substring(typingIndex, 8).Equals("<color=#") && splitTypingTexts[typingIndex + 14] == '>')
                    {
                    ...
                    if (splitTypingTexts[typingIndex] == '<' && typingIndex + 6 < text.Length &&
                        text.Substring(typingIndex, 6).Equals("<size="))
                    {
                    ...
                    for (int i = 0; i < richTextCloseSymbols.Length; i++)
                    {
                        ...
                        if (splitTypingTexts[typingIndex] == '<' &&
                            typingIndex + (1 + richTextCloseSymbols[i].Length) < text.Length && text
                                .Substring(typingIndex, 3 + richTextCloseSymbols[i].Length).Equals(closeSymbol))
                        {
                    ...
                    {
                        string convertedRichText = RichTextConvert(splitTypingTexts[typingIndex].ToString(), bold,
                            italic, size, fontSize, color, colorCode);
    ...
    private string RichTextConvert(string text, bool bold, bool italic, bool size, int fontSize, bool color,
        string colorCode)
    {
        StringBuilder stringBuilder = new StringBuilder();
